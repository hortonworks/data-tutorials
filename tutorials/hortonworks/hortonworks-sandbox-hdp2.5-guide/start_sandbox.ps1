#windows alternative for the Hortonworks start sandbox script

$sandbox_count = (docker ps -a -f=ancestor=sandbox| Measure-Object -Line).lines

if  ($sandbox_count -eq 1) {
	Write-Host creating new sandbox container
	...
	Write-Host starting existing sandbox container
	docker start sandbox
}
 
docker exec -t sandbox make --makefile /usr/lib/hue/tools/start_scripts/start_deps.mf  -B Startup -j -i
docker exec -t sandbox nohup su - hue -c '/bin/bash /usr/lib/tutorials/tutorials_app/run/run.sh'
